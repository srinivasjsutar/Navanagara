...
export function SiteBookingForm() {
  ...
  const projects = [
    ...
    { name: "Green Valley", code: "GVP" },
    { name: "Sunrise Heights", code: "SRH" },
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      ...
      try {
        const response = await axios.post("https://navagara-backend.onrender.com/site-booking", {
          name: values.Name,
          date: values.Date,
        ...
        if (error.response) {
          ...
          setSubmitMessage("No response from server. Please check if the server is running.");
  ...
  useEffect(() => {
    if (formik.values.ProjectName && seniorityInput) {
      const selectedProject = projects.find(p => p.name === formik.values.ProjectName);
      if (selectedProject) {
        ...
        // Fetch member details when seniority number is complete
  ...
  const fetchMemberDetails = async (seniorityNo) => {
    ...
    try {
      ...
      // Find member by seniority number
      ...
      if (member) {
        ...
        // Extract project name from the member's seniority number
  ...
  return (
    <div>
      ...
      <div className="w-[791px] px-10 ml-10">
        ...
        {submitMessage && (
          <div className={`p-4 mb-4 rounded ${submitMessage.includes("Error") || submitMessage.includes("error") || submitMessage.includes("not found") ? "bg-red-100 text-red-700" : "bg-green-100 text-green-700"}`}>
            {submitMessage}
          </div>
        )}
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              {fullSeniorityNo && (
                <div className={`text-sm mt-1 font-semibold ${memberFound ? 'text-green-600' : 'text-blue-600'}`}>
                  {isFetchingMember ? 'Checking...' : `Generated: ${fullSeniorityNo}`}
                </div>
              )}
